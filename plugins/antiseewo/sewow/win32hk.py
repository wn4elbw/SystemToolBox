...
advapi32.RegDeleteTreeW.argtypes = [HKEY, LPCWSTR, DWORD]
...
def query_service_status(hsvc):
    """查询服务状态。返回 SERVICE_STATUS 或 None。"""
    st = SERVICE_STATUS()
    ok = advapi32.QueryServiceStatus(hsvc, ctypes.byref(st))
    return st if ok else None


def service_state_name(state):
    return {
...
def stop_service_by_name(name):
    """按名字停止服务/驱动。返回 (ok, msg)。"""
    scm = open_sc_manager(SC_MANAGER_ALL_ACCESS)
    if not scm:
        return False, f"OpenSCManager 失败: {get_last_error()}"
    try:
        hsvc = open_service(scm, name, SERVICE_STOP | SERVICE_QUERY_STATUS)
        if not hsvc:
            err = get_last_error()
            if err == 1060:  # ERROR_SERVICE_DOES_NOT_EXIST
                return True, "服务不存在"
            return False, f"OpenService 失败: {err}"
        try:
            st = query_service_status(hsvc)
            if st and st.dwCurrentState == 1:  # STOPPED
                return True, "服务已停止"
            ok, st2 = control_service(hsvc, 1)  # SERVICE_CONTROL_STOP
            if not ok:
                return False, f"ControlService 失败: {get_last_error()}"
            return True, "已发送停止指令"
        finally:
            close_service_handle(hsvc)
    finally:
        close_service_handle(scm)


def delete_service_by_name(name):
    """停止并删除服务/驱动。返回 (ok, msg)。"""
    scm = open_sc_manager(SC_MANAGER_ALL_ACCESS)
    if not scm:
        return False, f"OpenSCManager 失败: {get_last_error()}"
    try:
        hsvc = open_service(scm, name,
        ...
        try:
            st = query_service_status(hsvc)
            if st and st.dwCurrentState != 1:
                control_service(hsvc, 1)  # 尝试停止（忽略结果）
            if not delete_service(hsvc):
                return False, f"DeleteService 失败: {get_last_error()}"
            return True, "服务已删除"
        finally:
            close_service_handle(hsvc)
    finally:
...
def reg_delete_tree(root, subkey, wow64=0):
    """递归删除注册表键树（管理员）。返回 (ok, msg)。"""
    rc = advapi32.RegDeleteTreeW(root, subkey, wow64)
    if rc in (0, 5):  # ERROR_SUCCESS / ERROR_ACCESS_DENIED
        return rc == 0, f"RegDeleteTree 返回 {rc}"
    return False, f"RegDeleteTree 失败: {rc}"


def reg_set_dword(hkey, name, value):
    """写 REG_DWORD。返回 0 表示成功。"""
    import struct
    buf = ctypes.create_string_buffer(struct.pack("<I", value))
    return advapi32.RegSetValueExW(hkey, name, 0, REG_DWORD, buf, 4)
